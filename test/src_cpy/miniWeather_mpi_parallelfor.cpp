...
void semi_discrete_step( realConst3d state_init , real3d const &state_forcing , real3d const &state_out , real dt , int dir , Fixed_data const &fixed_data ) {
  ...
  parallel_for($mem_freq, $core_freq,  SimpleBounds<3>(NUM_VARS,nz,nx) , YAKL_LAMBDA ( int ll, int k, int i ) {
...
void compute_tendencies_x( realConst3d state , real3d const &tend , real dt , Fixed_data const &fixed_data ) {
  ...
  // for (k=0; k<nz; k++) {
  //   for (i=0; i<nx+1; i++) {
  parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(nz,nx+1) , YAKL_LAMBDA (int k, int i ) {
...
void compute_tendencies_z( realConst3d state , real3d const &tend , real dt , Fixed_data const &fixed_data ) {
  ...
  // for (k=0; k<nz+1; k++) {
  //   for (i=0; i<nx; i++) {
  parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(nz+1,nx) , YAKL_LAMBDA (int k, int i) {
    SArray<real,1,4> stencil;
    SArray<real,1,NUM_VARS> d3_vals;
    SArray<real,1,NUM_VARS> vals;
...
void set_halo_values_x( real3d const &state , Fixed_data const &fixed_data ) {
  ...
  if (fixed_data.nranks == 1) {
    parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(NUM_VARS,nz) , YAKL_LAMBDA (int ll, int k) {
  ...
  //   for (k=0; k<nz; k++) {
  //     for (s=0; s<hs; s++) {
  parallel_for($mem_freq, $core_freq,  SimpleBounds<3>(NUM_VARS,nz,hs) , YAKL_LAMBDA (int ll, int k, int s) {
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      // for (k=0; k<nz; k++) {
      //   for (i=0; i<hs; i++) {
      parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(nz,hs) , YAKL_LAMBDA (int k, int i) {
...
void set_halo_values_z( real3d const &state , Fixed_data const &fixed_data ) {
  ...
  // for (ll=0; ll<NUM_VARS; ll++) {
  //   for (i=0; i<nx+2*hs; i++) {
  parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(NUM_VARS,nx+2*hs) , YAKL_LAMBDA (int ll, int i) {
    if (ll == ID_WMOM) {
      state(ll,0      ,i) = 0.;
      state(ll,1      ,i) = 0.;
      state(ll,nz+hs  ,i) = 0.;
...
void init( real3d &state , real &dt , Fixed_data &fixed_data ) {
  ...
  // for (k=0; k<nz+2*hs; k++) {
  //   for (i=0; i<nx+2*hs; i++) {
  parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(nz+2*hs,nx+2*hs) , YAKL_LAMBDA (int k, int i) {
  ...
  parallel_for($mem_freq, $core_freq,  nz+2*hs , YAKL_LAMBDA (int k) {
    hy_dens_cell      (k) = 0.;
  ...
  parallel_for($mem_freq, $core_freq,  nz+1 , YAKL_LAMBDA (int k) {
    real z = (k_beg + k)*dz;
    real r, u, w, t, hr, ht;
    if (data_spec_int == DATA_SPEC_COLLISION      ) { collision      (0.,z,r,u,w,t,hr,ht); }
    if (data_spec_int == DATA_SPEC_THERMAL        ) { thermal        (0.,z,r,u,w,t,hr,ht); }
...
void output( realConst3d state , real etime , int &num_out , Fixed_data const &fixed_data ) {
  ...
  // for (k=0; k<nz; k++) {
  //   for (i=0; i<nx; i++) {
  parallel_for($mem_freq, $core_freq,  SimpleBounds<2>(nz,nx) , YAKL_LAMBDA (int k, int i) {
